...
// ── Official Polyglot Random64[781] table ────────────────────────────────────
// Indices [0..767]   = piece[0..11] × square[0..63]
//         [768..771] = castling rights (WK, WQ, BK, BQ)
//         [772..779] = en-passant file 0..7
//         [780]      = side to move (XOR in when WHITE to move)
//
// Polyglot piece order: 0=bP 1=wP 2=bN 3=wN 4=bB 5=wB 6=bR 7=wR 8=bQ 9=wQ 10=bK 11=wK

static const uint64_t PolyRandom[781] = {
    // bP [0..63]
    ...
    // side to move [780]: XOR in when WHITE to move
    0xF8D626AAAF278509ULL
};

// ── Map our Piece enum → Polyglot piece index ─────────────────────────────────
// Polyglot: 0=bP 1=wP 2=bN 3=wN 4=bB 5=wB 6=bR 7=wR 8=bQ 9=wQ 10=bK 11=wK
static const int8_t kPolyIdx[13] = {
    -1,   // EMPTY
     1,   // WP
     3,   // WN
     5,   // WB
...
// ── Check whether an en-passant capture is actually possible ──────────────────
// Polyglot only mixes EP file into the key if a pawn can actually capture.
static bool epCapturePossible(const Board& b) {
    int ep = b.enPassantSquare;
    if (ep < 0) return false;
    int f = fileOf(ep);
    if (b.whiteToMove) {
        // WP can capture to ep: pawn must be at ep-9 (file f-1) or ep-7 (file f+1)
        if (f > 0 && b.squares[ep - 9] == WP) return true;
        if (f < 7 && b.squares[ep - 7] == WP) return true;
    } else {
        // BP can capture to ep: pawn must be at ep+7 (file f-1) or ep+9 (file f+1)
        if (f > 0 && b.squares[ep + 7] == BP) return true;
        if (f < 7 && b.squares[ep + 9] == BP) return true;
    }
    return false;
}

// ── Compute Polyglot hash for a board position ────────────────────────────────
static uint64_t polyHash(const Board& b) {
    uint64_t key = 0;
    ...
    for (int sq = 0; sq < 64; ++sq) {
        ...
        if (p == EMPTY) continue;
        int idx = kPolyIdx[p];
        key ^= PolyRandom[idx * 64 + sq];
    }

    // Castling (WK=768, WQ=769, BK=770, BQ=771)
    if (b.castlingRights[CR_WK]) key ^= PolyRandom[768];
    if (b.castlingRights[CR_WQ]) key ^= PolyRandom[769];
    if (b.castlingRights[CR_BK]) key ^= PolyRandom[770];
    if (b.castlingRights[CR_BQ]) key ^= PolyRandom[771];

    // En passant — only when a pawn can actually capture
    if (epCapturePossible(b))
        key ^= PolyRandom[772 + fileOf(b.enPassantSquare)];

    // Side to move: XOR in for WHITE (opposite of our internal convention)
    if (b.whiteToMove)
        key ^= PolyRandom[780];
...
bool bookLoad(const std::string& path) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        BookEntry e;
        e.key    = readBE64(buf);
        e.move   = readBE16(buf + 8);
        e.weight = readBE16(buf + 10);
        // buf[12..15] = learn, ignored
        if (e.weight > 0)
            g_book.push_back(e);
    }

    // Polyglot files are pre-sorted by key, but sort again to be safe
    std::sort(g_book.begin(), g_book.end(),
              [](const BookEntry& a, const BookEntry& b){ return a.key < b.key; });

    g_bookLoaded = !g_book.empty();
    // Use random_device for a non-deterministic seed so self-play games differ
    std::srand(std::random_device{}());
    return g_bookLoaded;
}

// ── Decode a Polyglot move to our Move (must be legal on board) ───────────────
static Move decodePolyMove(const Board& board, uint16_t polyMove) {
    int toSq   =  polyMove        & 0x3F;
    int fromSq = (polyMove >>  6) & 0x3F;
    int promo  = (polyMove >> 12) & 0x07;

    // Remap castling: Polyglot encodes king→rook, we use king→destination
    ...
    // Map promotion index to our Piece (1=N 2=B 3=R 4=Q)
    Piece promoPiece = EMPTY;
    if (promo) {
        bool white = board.whiteToMove;
        static const Piece wPromo[5] = { EMPTY, WN, WB, WR, WQ };
        static const Piece bPromo[5] = { EMPTY, BN, BB, BR, BQ };
        promoPiece = white ? wPromo[promo] : bPromo[promo];
    }

    // Match against legal moves
    std::vector<Move> legal = generateMoves(const_cast<Board&>(board));
    for (const Move& m : legal) {
        if (m.from == fromSq && m.to == toSq && m.promotion == promoPiece)
            return m;
    }
    return NULL_MOVE;
}
...
Move bookProbe(const Board& board) {
    if (!g_bookLoaded) return NULL_MOVE;

    uint64_t key = polyHash(board);

    // Binary search for first entry with this key
    auto it = std::lower_bound(g_book.begin(), g_book.end(), key,
        [](const BookEntry& e, uint64_t k){ return e.key < k; });

    if (it == g_book.end() || it->key != key) return NULL_MOVE;

    // Collect all entries with this key and compute total weight
    uint32_t total = 0;
    auto first = it;
    while (it != g_book.end() && it->key == key) {
        total += it->weight;
        ++it;
    }
    if (total == 0) return NULL_MOVE;

    // Weighted random selection
    uint32_t r = (uint32_t)std::rand() % total;
    uint32_t cumul = 0;
    for (auto jt = first; jt != it; ++jt) {
        cumul += jt->weight;
        if (r < cumul) {
            Move m = decodePolyMove(board, jt->move);
            if (!m.isNull()) return m;
        }
    }

    // Fallback: try each candidate until one is legal
    for (auto jt = first; jt != it; ++jt) {
        Move m = decodePolyMove(board, jt->move);
        if (!m.isNull()) return m;
    }
